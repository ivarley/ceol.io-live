...
class PersonFactory(factory.Factory):
    ...
    state = factory.LazyAttribute(
        lambda obj: next(
            (state for city, state in SESSION_CITIES if city == obj.city), "NY"
        )
    )
    country = "USA"
...
class UserAccountFactory(factory.Factory):
    ...
    username = factory.LazyAttribute(
        lambda obj: f"{fake.user_name()}{random.randint(1, 999)}"
    )
    ...
    is_active = True
    is_system_admin = factory.fuzzy.FuzzyChoice(
        [True, False], probability_true=0.1
    )  # 10% admins
    email_verified = factory.fuzzy.FuzzyChoice(
        [True, False], probability_true=0.9
    )  # 90% verified
...
class SessionFactory(factory.Factory):
    ...
    name = factory.LazyFunction(
        lambda: f"{random.choice(SESSION_CITIES)[0]} Irish Session"
    )
    ...
    location_name = factory.LazyAttribute(
        lambda obj: f"The {random.choice(['Celtic', 'Irish', 'Shamrock', 'Clover', 'Harp'])} {random.choice(VENUE_TYPES)}"
    )
    location_website = factory.LazyAttribute(
        lambda obj: f"https://{obj.location_name.lower().replace(' ', '').replace('the', '')}.com"
    )
    location_phone = factory.Faker("phone_number")
    location_street = factory.Faker("street_address")

    timezone = factory.LazyAttribute(
        lambda obj: TIMEZONE_MAP.get((obj.city, obj.state), "America/New_York")
    )
    ...
    unlisted_address = factory.fuzzy.FuzzyChoice(
        [True, False], probability_true=0.1
    )  # 10% unlisted
    ...
    termination_date = factory.LazyFunction(
        ...
        else fake.date_between(start_date="-1y", end_date="today")
...
class SessionInstanceFactory(factory.Factory):
    ...
    date = factory.LazyFunction(
        lambda: fake.date_between(start_date="-6m", end_date="+1m")
    )
    ...
    end_time = factory.LazyAttribute(
        lambda obj: datetime.combine(date.today(), obj.start_time)
        + timedelta(hours=random.choice([2, 3, 4]))
    )

    is_cancelled = factory.fuzzy.FuzzyChoice(
        [True, False], probability_true=0.05
    )  # 5% cancelled
    ...
    log_complete_date = factory.LazyFunction(
        ...
        else fake.date_time_between(start_date="-1m", end_date="now")
...
class SessionInstanceTuneFactory(factory.Factory):
    ...
    inserted_timestamp = factory.LazyAttribute(
        lambda obj: obj.played_timestamp + timedelta(minutes=random.randint(0, 30))
    )

    key_override = factory.LazyFunction(
        lambda: None
        if random.random() > 0.2
        else random.choice(["D", "G", "A", "C", "F", "Bb", "Eb"])
...
def generate_sample_session_instance_tunes(
    instance_ids, tune_ids, tunes_per_instance=8
):
