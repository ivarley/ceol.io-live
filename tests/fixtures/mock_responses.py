...
class MockTheSessionAPI:
    ...
    @staticmethod
    def get_tune_response(tune_id=1001, name="The Butterfly", tune_type="slip jig", tunebooks=156):
    ...
    @staticmethod
    def get_session_response(session_id=1, name="Austin Session", city="Austin", country="USA"):
...
class MockSendGridAPI:
    ...
    def get_failure_response(status_code=400, error_message="Bad Request"):
        ...
        mock_response.body = json.dumps({
            'errors': [{'message': error_message, 'field': 'to', 'help': 'Please check the email address.'}]
        }).encode()
        return mock_response
    ...
    def create_mock_sendgrid_client(success=True, status_code=202):
        ...
        if success:
            ...
            mock_client.send.return_value = MockSendGridAPI.get_failure_response(status_code)
...
class MockDatabaseResponses:
    ...
    def get_session_instance_data():
        ...
        return (
            ...
            1,  # session_id
            date(2023, 8, 15),  # date
            now_utc().replace(year=2023, month=8, day=15, hour=19, minute=30, second=0, microsecond=0),  # start_time
            now_utc().replace(year=2023, month=8, day=15, hour=22, minute=30, second=0, microsecond=0),  # end_time
    ...
    def get_tune_data():
        ...
        return (
            ...
            now_utc().replace(year=2023, month=1, day=1, hour=0, minute=0, second=0, microsecond=0),  # created_date
            now_utc().replace(year=2023, month=6, day=1, hour=0, minute=0, second=0, microsecond=0)  # last_modified_date
        )
    ...
    def get_session_instance_tune_data():
        ...
        return (
            ...
            1,  # order_number
            False,  # continues_set
            now_utc().replace(year=2023, month=8, day=15, hour=20, minute=15, second=0, microsecond=0),  # played_timestamp
            now_utc().replace(year=2023, month=8, day=15, hour=20, minute=20, second=0, microsecond=0),  # inserted_timestamp
            None,  # key_override
    ...
    @staticmethod
    def create_mock_cursor(fetchone_data=None, fetchall_data=None, execute_side_effect=None):
    ...
    def get_login_history_data():
        ...
        return [
            (
                ...
                None,  # failure_reason
                now_utc().replace(year=2023, month=8, day=15, hour=10, minute=30, second=0, microsecond=0),  # timestamp
            ...
            (
                ...
                now_utc().replace(year=2023, month=8, day=15, hour=9, minute=45, second=0, microsecond=0),  # timestamp
                None,  # additional_data
                None,  # first_name
                None  # last_name
            )
    ...
    def get_active_sessions_data():
        ...
        return [
            (
                ...
                'Test',  # first_name
                'User',  # last_name
                now_utc().replace(year=2023, month=8, day=15, hour=9, minute=0, second=0, microsecond=0),  # created_date
                now_utc().replace(year=2023, month=8, day=15, hour=12, minute=30, second=0, microsecond=0),  # last_accessed
                '127.0.0.1'  # ip_address
            ),
            (
                2,  # user_id
                'Another',  # first_name
                'User',  # last_name
                now_utc().replace(year=2023, month=8, day=15, hour=10, minute=15, second=0, microsecond=0),  # created_date
                now_utc().replace(year=2023, month=8, day=15, hour=12, minute=45, second=0, microsecond=0),  # last_accessed
    ...
    'invalid_registration_cases': [
        {
            ...
            'data': {'password': 'pass123', 'first_name': 'Test', 'last_name': 'User', 'email': 'test@example.com'},
            'error': 'required'
        },
        {
            'name': 'short_password',
            'data': {'username': 'test', 'password': 'short', 'confirm_password': 'short', 'first_name': 'Test', 'last_name': 'User', 'email': 'test@example.com'},
            'error': '8 characters'
        },
        {
            'name': 'password_mismatch',
            'data': {'username': 'test', 'password': 'password123', 'confirm_password': 'different123', 'first_name': 'Test', 'last_name': 'User', 'email': 'test@example.com'},
            'error': 'match'
        }
    ]
}
