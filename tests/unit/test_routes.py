...
class TestHomeRoute:
    ...
    def test_home_success(self, mock_get_conn, client):
        ...
        mock_cursor.fetchall.side_effect = [
            [(1, 'Austin Session', 'austin-session', 'Austin', 'TX', 'USA')],  # Active sessions
            [(datetime(2023, 8, 15).date(),), (datetime(2023, 8, 8).date(),)]  # Recent instances
        ]
...
class TestAuthenticationRoutes:
    ...
    @patch('web_routes.get_db_connection')
    def test_register_post_success(self, mock_get_conn, mock_get_user, client, mock_sendgrid):
    ...
    @patch('web_routes.User.get_by_username')
    def test_register_post_username_taken(self, mock_get_user, client, sample_user_data):
    ...
    @patch('web_routes.get_db_connection')
    def test_login_post_success(self, mock_get_conn, mock_login_user, mock_create_session,
...
class TestAdminRoutes:
    ...
    def test_admin_people_unauthorized(self, client):
        ...
        assert response.status_code == 302  # Redirect to login or due to lack of admin privileges
    ...
    def test_admin_people_authorized(self, mock_get_conn, client, admin_user):
        ...
        mock_cursor.fetchall.return_value = [
            (1, 'John', 'Doe', 'john@example.com', 'Austin', 'TX', 'USA', None,
             'johndoe', False, None, 2, 5, datetime(2023, 8, 15).date(), 'Test Session')
        ]
    ...
    def test_admin_sessions_page(self, mock_get_conn, client, admin_user):
        ...
        start_time = now_utc().replace(year=2023, month=8, day=15, hour=10, minute=0, second=0, microsecond=0)
        end_time = now_utc().replace(year=2023, month=8, day=15, hour=12, minute=0, second=0, microsecond=0)
        mock_cursor.fetchall.return_value = [
