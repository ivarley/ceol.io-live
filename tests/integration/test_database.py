...
class TestDatabaseConnections:
    ...
    def test_database_transaction_rollback(self, db_conn):
        ...
        cursor.execute("SELECT COUNT(*) FROM person WHERE first_name = %s", (f'TestRollback{unique_id}',))
        ...
        cursor.execute('''
            ...
            VALUES (%s, %s, %s)
        ''', (f'TestRollback{unique_id}', 'User', f'testrollback{unique_id}@example.com'))
        
        # Verify data exists before rollback
        cursor.execute("SELECT first_name FROM person WHERE first_name = %s", (f'TestRollback{unique_id}',))
...
class TestTuneMatching:
    ...
    def test_find_matching_tune_exact_alias(self, db_conn, db_cursor):
        ...
        matched_id, final_name, error = find_matching_tune(db_cursor, session_id, f'Session Alias Name {unique_id}')
    ...
    def test_find_matching_tune_by_name_with_the_prefix(self, db_conn, db_cursor):
        ...
        matched_id, final_name, error = find_matching_tune(db_cursor, session_id, search_name_without)
        ...
        matched_id, final_name, error = find_matching_tune(db_cursor, session_id, search_name_with)
    ...
    def test_find_matching_tune_session_tune_alias_table(self, db_conn, db_cursor):
        ...
        matched_id, final_name, error = find_matching_tune(db_cursor, session_id, f'Alias Table Name {unique_id}')
    ...
    def test_find_matching_tune_multiple_matches_error(self, db_conn, db_cursor):
        ...
        db_cursor.execute('''
            ...
            VALUES (%s, %s, %s, %s, %s)
            RETURNING session_id
        ''', (f'Multiple Match Session {unique_id}', session_path, 'Austin', 'TX', 'USA'))
        session_id = db_cursor.fetchone()[0]
        ...
        matched_id, final_name, error = find_matching_tune(db_cursor, session_id, alias_name)
    ...
    def test_find_matching_tune_no_match(self, db_conn, db_cursor):
        ...
        matched_id, final_name, error = find_matching_tune(db_cursor, session_id, 'Nonexistent Tune')
    ...
    def test_normalize_apostrophes_in_tune_search(self, db_conn, db_cursor):
        ...
        matched_id, final_name, error = find_matching_tune(db_cursor, session_id, search_name)
...
class TestComplexQueries:
    ...
    def test_session_with_instances_and_tunes(self, db_conn, db_cursor):
        ...
        db_cursor.execute('''
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING session_id
        ''', (f'Complex Query Session {unique_id}', session_path, 'Austin', 'TX', 'USA', 'America/Chicago'))
        session_id = db_cursor.fetchone()[0]
        ...
        for i, instance_id in enumerate(instance_ids):
            for j, (tune_id, name, _) in enumerate(tune_data[:2]):  # First 2 tunes per instance
                db_cursor.execute('''
        ...
        assert any(name in result[5] for name in [f'Complex Query Reel 1 {unique_id}', f'Complex Query Jig 1 {unique_id}'])  # most played tune
    ...
    def test_user_session_management_with_cleanup(self, db_conn, db_cursor):
        ...
        db_cursor.execute('DELETE FROM user_session WHERE expires_at < %s', (now_utc(),))
        db_conn.commit()
    ...
    def test_data_integrity_constraints(self, db_conn, db_cursor):
        ...
        with pytest.raises(Exception):  # Should raise foreign key error
            db_cursor.execute('''
                ...
                VALUES (%s, %s)
            ''', (99999, date(2023, 8, 15)))  # Non-existent session_id
            db_conn.commit()
