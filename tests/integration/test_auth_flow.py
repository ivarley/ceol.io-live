...
class TestRegistrationFlow:
    ...
    def test_complete_registration_flow(self, mock_send_email, client, db_conn, db_cursor):
    ...
    def test_registration_duplicate_username(self, client, db_conn, db_cursor, sample_user_data):
...
class TestLoginFlow:
    ...
    def test_successful_login_flow(self, client, db_conn, db_cursor):
        ...
        db_cursor.execute('''
            ...
            RETURNING user_id
        ''', (person_id, username, email, hashed_password,
              'UTC', True, True))
...
class TestPasswordResetFlow:
    ...
    def test_password_reset_completion(self, client, db_conn, db_cursor):
        ...
        db_cursor.execute('''
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            RETURNING user_id
        ''', (person_id, username, email, 'oldpass',
              reset_token, reset_expires, True))
        user_id = db_cursor.fetchone()[0]
        db_conn.commit()
    ...
    def test_password_reset_expired_token(self, client, db_conn, db_cursor):
        ...
        db_cursor.execute('''
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        ''', (person_id, username, email, 'oldpass',
              expired_token, expired_time, True))
        db_conn.commit()
        
        response = client.post(f'/reset-password/{expired_token}', data={
            'password': 'newpassword123',
            'confirm_password': 'newpassword123'
        })
...
class TestEmailVerificationFlow:
    ...
    @patch('web_routes.send_verification_email')
    def test_resend_verification_email(self, mock_send_email, client, db_conn, db_cursor):
        ...
        response = client.post('/resend-verification', data={
            'email': email
        })
...
class TestSessionManagement:
    ...
    def test_session_creation_and_cleanup(self, db_conn, db_cursor):
        ...
        db_cursor.execute('SELECT session_id FROM user_session WHERE user_id = %s', (user_id,))
