...
def test_email():
    try:
        ...
        if not api_key:
            return "SendGrid API key not configured. Please set SENDGRID_API_KEY environment variable.", 500
        
        # Generate random subject and message
        subjects = [
            "Test Email from Your Ceol.io App",
            "Random Test Message",
            "SendGrid Integration Working!",
            "Hello from Your Flask App",
            "Email Test Success"
        ]
        
        messages = [
            "This is a test email sent from your Flask application using SendGrid. If you're reading this, the integration is working perfectly!",
            "Congratulations! Your email functionality is now set up and ready to use.",
            "Random test message to verify email delivery through SendGrid API.",
            "Your app can now send emails! This test confirms everything is configured correctly.",
            "SendGrid integration successful. You can now implement email notifications in your application."
        ]
        ...
        # Use a verified sender email - you should verify this in SendGrid
        from_email = os.environ.get('SENDGRID_FROM_EMAIL', 'ceol@ceol.io')
        message = Mail(
            from_email=from_email,
            to_emails='ian@ianvarley.com',
            subject=random_subject,
            html_content=f'<p>{random_message}</p><br><p>Sent at: {random.randint(1000, 9999)}</p>'
        )
        
        sg = SendGridAPIClient(api_key)
        response = sg.send(message)
        
        return f"Email sent successfully!<br><br>Subject: {random_subject}<br>Message: {random_message}<br><br>SendGrid Response Code: {response.status_code}"
