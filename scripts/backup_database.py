...
def backup_table_data(cursor, table_name, file_handle):
    ...
    cursor.execute(
        sql.SQL("SELECT {} FROM {} ORDER BY 1").format(
            sql.SQL(", ").join(column_identifiers), sql.Identifier(table_name)
        )
    )
...
def create_backup():
    ...
    try:
        ...
        print(
            f"  History/audit tables ({len(history_tables)}): {', '.join(history_tables)}"
        )
        print()
        ...
        with open(backup_filename, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"--   Base tables ({len(base_tables)}): {', '.join(base_tables)}\n"
            )
            f.write(
                f"--   History/audit tables ({len(history_tables)}): {', '.join(history_tables)}\n"
            )
            f.write("-- \n")
            f.write(
                "-- NOTE: History tables contain audit trails for undo/diff functionality.\n"
            )
            f.write(
                "--       All base tables have created_date and last_modified_date columns.\n"
            )
            f.write("-- \n\n")
