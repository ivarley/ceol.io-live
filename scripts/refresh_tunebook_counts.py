...
def refresh_single_tune_count(tune_id):
    ...
    try:
        ...
        if response.status_code != 200:
            return (
                False,
                f"Failed to fetch data from thesession.org (status: {response.status_code})",
                None,
                None,
            )
...
def main():
    ...
    if test_mode:
        ...
        if len(tunes) > 10 and not skip_confirm:
            ...
            response = input(
                f"This will process {len(tunes)} tunes (estimated {estimated_time}+ minutes). Continue? (y/N): "
            )
